...
class OsTemplate < ActiveRecord::Base
  ...
  def self.get_available_official
    get_available(OWP.config.os_templates.mirror.path + '/precreated/')
  end

  def self.get_available_contributed
    get_available(OWP.config.os_templates.mirror.path + '/precreated/contrib/')
  end

  def self.install_official(hardware_server, name)
    self.download(hardware_server, OWP.config.os_templates.mirror.path + '/precreated/', name)
  end

  def self.install_contributed(hardware_server, name)
    self.download(hardware_server, OWP.config.os_templates.mirror.path + '/precreated/contrib/', name)
  end
  ...
  def self.get_available(dir)
    ftp = Net::FTP.new(OWP.config.os_templates.mirror.host)
    ftp.login('anonymous', 'anonymous')
    ftp.passive = true if OWP.config.os_templates.passive_ftp
    ftp.chdir(dir)
  ...
  def self.download(hardware_server, path, name)
    hardware_server.rpc_client.job('wget', "-P #{hardware_server.templates_dir}/cache/ ftp://" +
      OWP.config.os_templates.mirror.host + "/#{path}/#{name}.tar.gz")
  end
