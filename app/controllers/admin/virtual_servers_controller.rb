...
class Admin::VirtualServersController < Admin::Base
  ...
  def show
    ...
    @migration_targets = HardwareServer.
      where("id != ?", @virtual_server.hardware_server.id).
      map do |server|
      ...
      params = [
        {
          ...
          :value => "<img src=\"#{base_url}/assets/" + (('running' == virtual_server.real_state) ? 'run' : 'stop') + '.png"/>',
        }, {
